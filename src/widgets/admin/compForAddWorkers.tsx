...
export default function EmployeeEvaluationForm({setInfoAddUser}: any) {
  ...
  const [professionalSkills, setProfessionalSkills] = useState<ProfessionalSkill[]>([
    { name: "Навык 1", rating: 0 },
    { name: "Навык 2", rating: 0 },
  ])
  

  const [potentialPosition, setPotentialPosition] = useState("")
  const [areasDevelop, setAreasDevelop] = useState("")
  const [srokPov, setSrokPov]=useState('')
  const [attestationResult, setAttestationResult] = useState("")  
  const [compLevel, setCompLevel] = useState("") 
  const [projects, setProjects] = useState<Project[]>([
    { name: "Проект 1", description: "" },
  ])
  ...
  const addProject = () => {
    setProjects([...projects, { name: `Проект ${projects.length + 1}`, description: "" }])
  }
  ...
  const ratingLabels = [
   ...
   useEffect(() => {
      ...
      projects: projects.map((project)=> project.description)
      }));
            ...
            <Typography variant="h6" gutterBottom>
              Участие в проектах и достигнутые результаты:
                ...
                <div key={index} className='flex flex-col  gap-[10px]'>
						<p className='text-[17px] tracking-[1px] '>{project.name}</p>
