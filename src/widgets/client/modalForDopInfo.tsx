...
export default function ModalForDopInfo({dopInfo, setOpenDopInfo}: any ) {
...
const [certificate, setCertificate] = React.useState<{ name: string; dateReceived: string }[]>([
 ...
 React.useEffect(()=>{
	...
	if(userData?.docName){
		setCertificate(userData?.docName)
	}
 ...
 }, [userData?.courses, userData?.trainings, userData?.programs, userData?.hobbies, userData?.docName, userData?.careerGoals, userData?.сareerGoals, userData?.willToLearn,userData?.willToTravel, userData?.willToRotateChangePositions, userData?.publicAndCorporate])

const handleCertificateChange = (index: number, key: "name" | "dateReceived", value: string) => {
...
const removeCourse = (index: number) => {
  ...
  "hobbies": hobby
	...
	console.log(userDopInfo);
	updateUser(userDopInfo)
	setOpenDopInfo(false)
  }
