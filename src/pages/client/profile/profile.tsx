...
const Profile = () => {
	...
	const handleRout=()=>{
		toast.success('Добро пожаловать в админскую часть! ')
         navigate('/admin')
	}
...
if (isLoading) {
  ...
  <section className='w-[80%] m-auto overflow-hidden'>
   <div className='flex items-center gap-[100px]'>
		<div className='flex gap-[70px] w-[55%] items-center'>
  ...
  <div key={item.id} className='flex flex-col items-center gap-[10px]' onClick={() => downloadFile(`${API}/${item.url}`, item.originalName)}> 
      ...
      <p>{formatFileName(item.originalName)}</p>
           ...
           <ModalForDopInfoEdit/>
			...
			{
				btnShow ? <GreenButton value={'Админская часть'} shirina={200} func={handleRout}/> : ''
			}
			</div>
