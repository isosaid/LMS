...
export default function LanguageProficiencyFormAdd<T extends Record<string, any>>({
  setInfoAddUser
  ...
  // Синхронизация с родительским стейтом
  useEffect(() => {
    setInfoAddUser((prev) => ({
      ...prev,
  ...
  const handleLanguageChange = (id: number, value: string) => {
    setLanguages((prev) =>
      prev.map((lang) => (lang.id === id ? { ...lang, language: value } : lang))
    )
  }
